/* ============================================================
   SITE CONTENT - single source of truth for every word on the site.

   You should not need to edit this by hand. Open admin.html, make
   your changes there, and hit Publish.

   Sections whose item list is empty are hidden on the live site, and
   their nav links disappear with them. That is how the case studies,
   teardowns, experiments, observations, frameworks and learning
   sections stay out of the way until you have written something.
   ============================================================ */

window.SITE_CONTENT = {
  "meta": {
    "schemaVersion": 2,
    "updatedAt": "2026-08-06"
  },
  "config": {
    "siteUrl": "https://atandritc.github.io/Personal-Portfolio-Website/",
    "resumePath": "assets/resume.pdf",
    "resumeDownloadName": "Atandrit_Chatterjee_Resume.pdf",
    "resumeUpdated": "Aug 2026",
    "web3formsKey": "7f4fa5b9-795d-47ad-b1ca-0b6357cbbb1f",
    "calendlyUrl": ""
  },
  "availability": {
    "show": true,
    "status": "open",
    "headline": "Open to work",
    "detail": ""
  },
  "profile": {
    "name": "Atandrit Chatterjee",
    "initials": "AC",
    "title": "Aspiring Product Manager",
    "transition": "Associate Software Engineer → Aspiring Product Manager",
    "location": "Pune, India",
    "email": "atandrit04chatterjee@gmail.com",
    "phone": "+91 95064 11839",
    "photo": "assets/pfp.jpg",
    "greeting": "Atandrit Chatterjee",
    "summary": "I turn half-formed requirements into things that ship. For nearly two years at Amdocs I sat between nine international telecom clients and the engineering team, and the part I kept reaching for was the part before the code: who has this problem, how do we know, and what do we build first. I'm looking for an Associate Product Manager role where that is the job rather than the margin of it."
  },
  "seo": {
    "title": "Atandrit Chatterjee — Aspiring Product Manager",
    "description": "Software engineer moving into product management. Nearly two years at Amdocs across 9 international telecom accounts, plus two products built solo from problem to live deployment. Looking for an Associate Product Manager role.",
    "keywords": "Aspiring Product Manager, Associate Product Manager, APM, Product Management, Career Transition, Requirement Gathering, Scoping, Prioritisation, SQL, Agile, Software Engineer to Product Manager, Atandrit Chatterjee",
    "ogImage": "assets/og-image.png"
  },
  "metrics": [
    {
      "value": "9",
      "label": "International accounts",
      "detail": "Technical point of contact across Vodafone, Vivo, PLDT, VEON and Azercell"
    },
    {
      "value": "~2 yrs",
      "label": "Shipping in production",
      "detail": "Amdocs, Nov 2024 – Jul 2026, on live telecom systems"
    },
    {
      "value": "2",
      "label": "Products built solo",
      "detail": "StatReads and Cadence, from problem definition to live deployment"
    },
    {
      "value": "10+",
      "label": "Incidents resolved",
      "detail": "First-line production support with root-cause analysis"
    }
  ],
  "whyPm": {
    "heading": "How I got here",
    "paragraphs": [
      "Requirements at Amdocs arrived half-formed more often than not. The conversation that followed was usually about implementation — which service to touch, what the schema should look like — and somewhere in it the question of what the client was actually trying to do would quietly go missing. I got into the habit of going back and asking.",
      "That habit is where the useful work came from. Client teams were processing records one at a time because that was what the system allowed, and nobody had raised a defect, because nothing was technically broken. Bulk operations weren't on anyone's list until I went and watched how people were using the thing.",
      "Two years of that convinced me the decisions that matter most sit before the ticket rather than inside it. StatReads and Cadence are what happened when I started making those calls myself, with nobody assigning them and no spec to work from."
    ]
  },
  "about": {
    "heading": "About me",
    "paragraphs": [
      "I'm a software engineer. For nearly two years at Amdocs I worked across nine international telecom accounts as the person clients came to when a requirement was still unclear — sitting with them, working out what they actually needed, and translating it into something the team could build without a week of back-and-forth.",
      "I haven't held a product title, and I'm not going to pretend otherwise. What I've done instead is run the whole loop myself on my own time. StatReads and Cadence are products I took from \"I think this problem is real\" through design decisions, deliberate scope cuts, and a live deployment. Nobody assigned them and nobody wrote the spec — that was the point."
    ]
  },
  "apmPitch": {
    "heading": "Why hire me for an APM role",
    "intro": "Everything here is checkable — against something on this page, or against something you can ask me about.",
    "lackHeading": "What I don't have",
    "lack": [],
    "bringHeading": "What I bring",
    "bring": [
      "Two years shipping production software for nine international telecom clients, so I know the cost of what I'm asking engineers for when I ask for it.",
      "The client-facing half of the job already: requirement discovery, demos, user training, and being the person accounts escalate to.",
      "A shipped record outside work. StatReads and Cadence went from problem to live deployment with nobody assigning them.",
      "Comfort with data. I would rather answer a question with a SQL query than with an opinion.",
      "Learning velocity you can check rather than take on trust — this site, and everything on it, is self-directed."
    ]
  },
  "principles": {
    "heading": "How I think about product",
    "intro": "These come from two years of engineering rather than from managing a product. They're the things I've watched actually matter.",
    "items": [
      {
        "title": "Talk to the person with the problem",
        "body": "Requirements arrived vague at Amdocs because the person writing them was often guessing too. The fastest way past that was always a conversation, never a longer document."
      },
      {
        "title": "Decide with evidence, not volume",
        "body": "I'd rather run one SQL query against real usage than win an argument in a meeting. When there's no data yet, I say so out loud instead of dressing a guess up as a finding."
      },
      {
        "title": "Scope is the real decision",
        "body": "Almost anything is buildable. What you're actually choosing is what to leave out. Cadence only reached a live deployment because I cut it to sign-in and one core workflow."
      },
      {
        "title": "Trust is a feature",
        "body": "In StatReads I spent time showing users which API supplied each field instead of adding another chart. If people don't believe the numbers, the numbers don't matter."
      }
    ]
  },
  "experience": {
    "heading": "Experience",
    "items": [
      {
        "role": "Associate Software Engineer",
        "company": "Amdocs",
        "location": "Pune, India",
        "start": "Nov 2024",
        "end": "Jul 2026",
        "summary": "An engineering role, but a client-facing one: I was the person accounts came to when a requirement was still unclear.",
        "scope": "Vodafone (Netherlands, Ireland, Italy, Ukraine) · Vivo · PLDT · VEON (Kar-Tel, Unitel) · Azercell · B2B-Core reusable modules",
        "bullets": [
          "Acted as technical point of contact for nine international telecom accounts, sitting between client teams, product owners and engineering to turn half-formed requirements into specs the team could build from.",
          "Identified that client teams were processing records one at a time, proposed bulk operations instead, and saw it through — cutting turnaround time and removing a recurring source of manual data-entry errors.",
          "Owned first-line production support, resolving 10+ incidents through root-cause analysis and feeding what I found back into what we built next.",
          "Coordinated releases across lower and production environments with product, QA and infrastructure — sequencing dependencies, planning go-lives, and reducing rollback risk.",
          "Drove adoption of what we shipped through SQL-based analysis, client demos, user training, and runbook and knowledge-base documentation."
        ],
        "tags": ["Requirement Gathering", "Client Communication", "Release Management", "SQL", "Incident Management", "Product Demos"]
      }
    ]
  },
  "projects": {
    "heading": "Products I've built end to end",
    "intro": "Two products I built on my own time, partly so I'd have something real to point at. For each one the interesting part isn't the stack — it's what I chose not to build.",
    "items": [
      {
        "title": "StatReads",
        "subtitle": "Reading analytics dashboard",
        "role": "Solo — problem definition, design, build, ship",
        "status": "Live",
        "problem": "Readers log hundreds of books on Goodreads and StoryGraph and get almost nothing back. The data exists; the insight doesn't. There was no way to see the cross-cutting patterns — which decades you gravitate toward, how your genre mix has shifted, where in the world your authors actually come from.",
        "user": "",
        "evidence": [],
        "decisions": [
          "Chose CSV import over an integration. StoryGraph has no public API and Goodreads retired theirs, so import was the only path that worked on day one — and it kept the app fully client-side, with no accounts and no data leaving the browser.",
          "Prioritised trust over feature count. Public metadata is frequently wrong or missing, so instead of adding another chart I built a sourcing table showing which API supplied each field.",
          "Let readers fix the data rather than just flagging it. Wrong or missing publication years, countries, genres, series details, covers and author photos can each be overridden by hand, and the correction outranks the API on every later refetch.",
          "Made those corrections compound. A fix can be submitted upstream into a shared file that ships with the app, so one reader correcting a bad publication year fixes it for everyone instead of every user repeating the same work.",
          "Layered three APIs behind a fallback pipeline (Open Library, then Google Books, then Wikidata) to maximise coverage, rather than picking one and living with the gaps."
        ],
        "outcome": "10+ interactive visualisations across genre, decade, theme, author and geography — including a choropleth map. Runs entirely in the browser: no backend, no signup, no data collection.",
        "metrics": [],
        "risks": [],
        "tags": ["JavaScript", "HTML5 Canvas", "Open Library API", "Google Books API", "Wikidata"],
        "live": "https://atandritc.github.io/StatReads",
        "github": "https://github.com/atandritC/StatReads"
      },
      {
        "title": "Cadence",
        "subtitle": "Project management REST API",
        "role": "Solo — modelling, API design, deployment",
        "status": "Live",
        "problem": "I wanted to find out what it actually takes to model collaborative work — projects, tasks, assignment, progress — rather than implement endpoints from a spec somebody else had already thought through.",
        "user": "",
        "evidence": [],
        "decisions": [
          "Started from the user's job, not the data model. I worked backwards from how a team creates a project, assigns work and checks progress, and let that decide what to model and expose.",
          "Treated developer experience as the product. Swagger/OpenAPI documentation and consistent, readable error responses were in scope from the start, because an API nobody can figure out isn't really shipped.",
          "Cut scope deliberately to reach 'live' — sign-in and the core workflow only, deployed to the cloud — instead of a more complete version that never left my laptop."
        ],
        "outcome": "Running publicly with interactive Swagger docs and secured sign-in, so anyone can try the API in the browser without cloning anything.",
        "metrics": [],
        "risks": [],
        "tags": ["Java", "Spring Boot", "PostgreSQL", "REST", "Swagger / OpenAPI", "Render"],
        "live": "https://cadence-t6ui.onrender.com/swagger-ui.html",
        "github": "https://github.com/atandritC/Cadence"
      }
    ]
  },
  "caseStudies": {
    "heading": "Product case studies",
    "intro": "Products I don't work on, taken apart and put back together: what's broken, who it's broken for, what the evidence says, and what I'd ship first.",
    "items": []
  },
  "teardowns": {
    "heading": "Product teardowns",
    "intro": "Products I use constantly, looked at properly — value proposition, user journey, growth loops, and what I'd cut.",
    "items": []
  },
  "experiments": {
    "heading": "Experiments",
    "intro": "Small things I built to test an assumption, including the ones that didn't work.",
    "items": []
  },
  "observations": {
    "heading": "Product observations",
    "intro": "One thing I noticed about a product, written down as I notice it.",
    "items": []
  },
  "frameworks": {
    "heading": "Frameworks I reach for",
    "intro": "What I use, and what I use it for.",
    "items": []
  },
  "learning": {
    "heading": "What I'm learning",
    "intro": "What I'm working on right now.",
    "items": []
  },
  "skills": {
    "heading": "Skills",
    "groups": [
      {
        "group": "Product-Adjacent",
        "items": ["Requirement Gathering", "Client & Stakeholder Conversations", "Problem Framing", "Scoping & Trade-offs", "Writing Specs & Documentation", "User Stories & Acceptance Criteria", "Usability & User Needs", "End-to-End Ownership"]
      },
      {
        "group": "Data & Decisions",
        "items": ["SQL", "Data Analysis", "Log Analysis", "Root Cause Analysis", "Evidence-Based Decisions"]
      },
      {
        "group": "Delivery",
        "items": ["Agile / Scrum", "Release Management", "Cross-Functional Coordination", "Production Support", "Incident Management", "SLAs"]
      },
      {
        "group": "Customer-Facing",
        "items": ["Client Onboarding", "Product Demos", "User Training", "Requirement Workshops", "Written & Verbal Communication"]
      },
      {
        "group": "Tools",
        "items": ["Jira", "Confluence", "Postman", "DBeaver", "Figma", "Git", "MobaXterm", "Cursor", "ChatGPT"]
      },
      {
        "group": "Technical",
        "items": ["Core Java", "Spring Boot", "REST APIs", "JSON / XML", "PostgreSQL", "JavaScript", "Linux / Unix", "Debugging"]
      }
    ]
  },
  "writing": {
    "heading": "Product writing",
    "intro": "Teardowns, specs and product thinking in long form.",
    "items": []
  },
  "education": {
    "heading": "Education",
    "items": [
      {
        "school": "SRM Institute of Science and Technology (SRMIST)",
        "degree": "B.Tech. in Computer Science, specialisation in Information Technology",
        "location": "Kattankulathur, Tamil Nadu",
        "start": "Aug 2020",
        "end": "May 2024",
        "detail": ""
      }
    ]
  },
  "certifications": {
    "heading": "Courses & certifications",
    "items": []
  },
  "socials": [
    {
      "label": "Email",
      "handle": "atandrit04chatterjee@gmail.com",
      "url": "mailto:atandrit04chatterjee@gmail.com"
    },
    {
      "label": "LinkedIn",
      "handle": "linkedin.com/in/atandrit-chatterjee",
      "url": "https://www.linkedin.com/in/atandrit-chatterjee/"
    },
    {
      "label": "GitHub",
      "handle": "github.com/atandritC",
      "url": "https://github.com/atandritC"
    }
  ],
  "contact": {
    "heading": "Get in touch",
    "blurb": "If you're hiring an APM and open to someone making the switch from engineering, I'd like to hear from you. I reply to everything.",
    "directHeading": "Or reach me directly",
    "successMessage": "Thanks for reaching out — I'll get back to you within a day."
  },
  "footer": {
    "note": "Built and maintained by Atandrit Chatterjee"
  }
};
